...
## 方块实体执行顺序逐帧对照（`java26_2BlockEntityOrder`）

`Level.tickBlockEntities` 按 `Level.blockEntityTickers` 的列表顺序迭代，所以那份列表**就是**
执行顺序。它是一个普通 `List`，捕获器只读地反射它并跳过已移除的条目
（与游戏迭代时的过滤一致）。内核侧把同一件事建模成每个方块实体一个单调注册序号，
对应 `blockEntityOrderJson()`。

`captureBlockEntityOrder.py` → `java26_2BlockEntityOrder`：18 刻 / 9 点。
覆盖按放置顺序入列、稍后放置的追加到末尾、移除后重新放置**回到末尾**、
同一格换成另一种方块实体、以及同刻放置两个时的相对顺序。
空唱片机作为反例：它有方块实体但没有 ticker，因此**不应该**出现在列表里，两侧一致。

**注意**：这类场景里不能放阳光探测器。它的读数取决于世界时钟，而每次捕获的时钟不同，
会让整个场景不可复现；测试结构的屏障外壳并不挡天空光。
...
- 轨迹只包含**邻居更新与形状更新**的序列。方块计划刻与方块事件、随机源消耗、
  方块实体执行顺序各有独立对照（见上面三节）。这四条对照都只在开启它的场景里采集，
  其余场景仍然只比较逐刻末状态。

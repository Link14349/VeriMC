...
最新一轮（`vanillaAll7`）的结论记在
[vanillaOnlyReplayResults.json](vanillaOnlyReplayResults.json)：
**42/42** 与 GameTest 捕获逐字段一致（含刻内更新轨迹与掉落物观测），42/42 通过 `checkReference`，
4 个带轨迹的场景在关掉轨迹后帧序列完全相同且原点相同。
其中两个区块生命周期场景本来就由这台服务器在选定的对齐原点上产出，
它们的「与 GameTest 捕获一致」一栏比较的是同一台服务器的两次捕获。

## 这一轮**没有**证明什么

- 只覆盖这 42 条时间线记录的观测字段。开关等价是**在这些场景上**成立，不是全局证明。

...
正在播放的唱片机（方块实体计时）、以及一个刚被振动事件激活的幽匿感测体（传播/冷却倒计时）。
...
`BlockTicks::chunkAt` 的向下取整）两个场景，60 刻 / 24 点，全部逐帧一致。
...
- 漏斗冷却、唱片机播放计时、幽匿感测体倒计时都有原版差分。
  **活塞运动进度**在停摆期间的冻结没有单独覆盖（场景里的活塞是在停摆前后完成运动的，
  没有构造出「运动进行到一半时区块停摆」的历史）。

...
- 点击视图右上角“第一人称”进入飞行搭建；“全屏”或 `F11` 进入全屏。`E` 打开可搜索、分类的物品列表，选择物品后放入九格快捷栏，按 `Esc` 或非输入框中再按 E 关闭并直接恢复搭建。在世界中按 `Esc` 才释放鼠标、显示“继续搭建”。详见 [第一人称交互](docs/creativeInteraction.md)。
- 第一人称中鼠标转向，`WASD` 移动，`Ctrl` 加速；左键拆除，右键优先使用器件，`Shift` + 右键贴面放置；`1–9` / 滚轮换槽，中键取物。准星交互距离为 20 格。按 Esc 释放鼠标后，可用右上角“碰撞与重力”开关设置移动方式（默认关闭）：关闭时无重力自由飞行；开启后受重力、Space 跳跃，双击 Space 切换飞行，飞行仍保留碰撞。行走时 Shift 不下降，飞行时 Space / Shift 升降。实体碰撞支持滑墙并阻止在身体内放置方块；全屏和窗口第一人称共用设置。碰撞与运动采用编辑器简化规则，尚非完整原版玩家物理。
- 开启碰撞时，飞行落到方块顶面会自动退出飞行并恢复重力行走；撞墙或撞到头顶不会退出。落地后单按 Space 跳跃，双击 Space 可再次起飞。
- 开启碰撞行走时，会自动跨过不超过 0.6 格的低坎；遇到一格高的方块且头顶有空间时自动跳跃。按住 Shift 可暂时抑制自动跳跃。中继器和比较器按 1/8 格底板碰撞，阳光探测器为 3/8 格、两种幽匿感测体为 1/2 格，上方装饰不再挡路。
- 初始样例：拉杆经过两级中继器控制红石灯；按钮驱动铜灯记忆电路。
- [8 位加法器 JSON 样例](examples/README.md)：17 根输入拉杆、8 位和与进位灯，默认 37 + 19 = 56，可从工程菜单导入。
- 默认进入模式 `2`（搭建）；左键拆除，右键优先操作按钮、拉杆等可交互器件，否则贴面放置。`Shift` + 右键绕过操作直接放置，空处落在当前 Y 层。模式 `4`（操作）的左右键都执行相同的器件操作，容器打开库存/环境面板。右键拖动仍只旋转视角。右侧可铺底板和调整方向，`R` 旋转。
- 选择器件库里的新方块会清除旧选择，右侧切换到放置设置。`R` 只旋转待放置器件；`/` 聚焦搜索，Esc 逐层关闭帮助或菜单后清除选择。输入框和帮助弹窗内不会触发背景运行快捷键。
- 放置预览复用场景中的简化模型；中继器/比较器箭头指向输出，侦测器箭头指向检测面。网格跟随编辑 Y 层。预览不代表支撑或接线已通过检查，真实放置仍由内核验证。
- “剖切”可按 X/Y/Z 显示不超过指定坐标的区域；隐藏区域不会被选中或放置。剖切仅改变显示，不改变方块或仿真，空处放置使用当前 Y 层，表面相邻放置受剖切范围约束。
- 器件库区分“已实现 / 部分实现 / 需环境输入”；标签描述实现范围，不代表完整原版兼容验证。新版内核直接提供器件默认属性；旧内核缺少对应状态时显示默认提示并暂不显示模型预览。
- 右键点击放置、拖动旋转（拖动后松手不会放置），中键平移、滚轮缩放。点击三维工作台后，可用 `W A S D` 按镜头朝向在水平面前后左右移动，`Space` 上升、`Shift` 下降，长按连续移动（12 格/秒，斜向不加速）；编辑输入框或下拉框时按键保持原有用途。支持俯视、适合画面和编辑层剖切。
- 工作台快捷键：`1` 选择；`2` 搭建；`3` 探针；`4` 操作拉杆、按钮等。`Enter` 运行/暂停，`F` 前进一个游戏刻。
- 运行中放置、拆除、批量编辑、撤销与重做保持运行；手动暂停后编辑也保持暂停。不限速模式在电路稳定后仍保持运行、等待新事件，仿真时间停在最后实际事件刻；底部“事件/s”统计连续运行的实际计划事件，不把空闲时间跳跃算成处理速度。
- 属性面板检查坐标、方块状态和信号强度；中继器、比较器的朝向属性指向输入。
- 波形支持缩放、跟随、双游标和触发暂停；探针菜单的 `↑` 或 `↓` 设置边沿断点。Shift 单击波形放置 B 游标。
- 浏览器接收滞后、未确认采样满额时自动暂停并保留后续事件；接收完成后可继续运行。多个模拟器页面共享同一仿真，任一页面超过 2 秒未确认数据都会触发暂停；仍能接收的页面会独立收到暂停原因。历史截断会显示数量，具体预算和极端中止条件见 [采样传输](docs/traceTransport.md)。

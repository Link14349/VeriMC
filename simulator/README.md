...
- “剖切”可按 X/Y/Z 显示不超过指定坐标的区域；隐藏区域不会被选中或放置。剖切仅改变显示，不改变方块或仿真，编辑仍位于当前 Y 层。
- 器件库区分“已实现 / 部分实现 / 需环境输入”；标签描述实现范围，不代表完整原版兼容验证。新版内核直接提供器件默认属性；旧内核缺少对应状态时显示默认提示并暂不显示模型预览。
- 右键拖动旋转、中键平移、滚轮缩放。点击三维工作台后，可用 `W A S D` 向上、左、下、右平移视角，速度为原方向键的 3 倍，长按连续平移；编辑输入框或下拉框时按键保持原有用途。支持俯视、适合画面和编辑层剖切。
- `1` 选择；`3` 移除；`4` 放置探针；`5` 操作拉杆、按钮等。Space 运行/暂停，`F` 前进一个游戏刻。

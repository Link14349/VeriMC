...
export function Waveform({ status, command }: { status: Status; command: (cmd: string, body?: Record<string, unknown>) => Promise<Record<string, unknown> | undefined> }) {
  ...
  const [traceDropped, setTraceDropped] = useState(0);
  ...
  useEffect(() => {
    ...
    draw.current = () => {
      ...
      s.probes.forEach((probe,index) => {
        ...
        const visible = connection.trace.window(probe.id, left, left+windowSpan); let last=visible.initial; let initialized=last!==undefined;
        ctx.strokeStyle=probeColors[index%probeColors.length];ctx.lineWidth=1.7;ctx.beginPath();
        if (initialized) ctx.moveTo(x(visible.start),bottom-last!/15*25);
        for (let i=visible.begin;i<visible.end;++i) {
          const [,tick,,value]=visible.rows[i];
          if (!initialized) { ctx.moveTo(x(tick),bottom-value/15*25); initialized=true; }
          else ctx.lineTo(x(tick),bottom-last!/15*25);
          ctx.lineTo(x(tick),bottom-value/15*25);last=value;
        }
        if (initialized) ctx.lineTo(Math.min(width,x(s.tick)),bottom-last!/15*25);ctx.stroke();
        if (!initialized) {ctx.fillStyle='#647177';ctx.fillText('该时段无采样',12,top+7);}
      });
    ...
    let frame = 0;
    const schedule = () => { if (!frame) frame = requestAnimationFrame(() => { frame = 0; setTraceDropped(connection.trace.dropped); draw.current(); }); };
    const observer = new ResizeObserver(schedule);observer.observe(wrap);connection.addEventListener('trace',schedule);
    return () => { observer.disconnect();connection.removeEventListener('trace',schedule);cancelAnimationFrame(frame); };
  },[]);
  ...
  return <section className="waveform">
    ...
    <div className="waveFooter"><span>游戏刻 gt · 1 gt = 50 ms · 保留同刻边沿顺序</span><span>{cursors[0]!==null&&cursors[1]!==null?`Δ ${Math.abs(cursors[1]-cursors[0])} gt / ${Math.abs(cursors[1]-cursors[0])*50} ms`:'A —     B —     Δ —'}</span>{traceDropped>0&&<span className="warning">显示历史已截断 {traceDropped} 条</span>}</div>
  </section>;
}

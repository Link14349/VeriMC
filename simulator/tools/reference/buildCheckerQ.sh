...
"$cmakeBin" -S simulator -B simulator/buildQ -DCMAKE_BUILD_TYPE=Release -DsimulatorBuildServer=OFF -DBUILD_TESTING=ON "-DCMAKE_PREFIX_PATH=$prefix"
"$cmakeBin" --build simulator/buildQ --target checkReference -j 6

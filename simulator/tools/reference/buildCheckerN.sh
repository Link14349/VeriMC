...
"$cmakeBin" -S simulator -B simulator/buildN -DCMAKE_BUILD_TYPE=Release -DsimulatorBuildServer=OFF -DBUILD_TESTING=ON "-DCMAKE_PREFIX_PATH=$prefix"
"$cmakeBin" --build simulator/buildN --target checkReference -j 6

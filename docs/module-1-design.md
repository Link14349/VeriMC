...
用户于 2026-09-07 确定默认二进制工程扩展名为 **`.vmcb`**。具体格式见 [VMCB 1.0 设计稿](../simulator/docs/vmcbFormat.md)：文件级状态表、16³ 分区、自适应布局编码、独立 Zstandard 压缩及带校验的目录；元数据使用 CBOR。目前完成格式设计，读写接入尚未实现，现有程序仍使用 JSON。
...
设计可用稳定文本表示导出用于版本控制和审阅；默认二进制布局不逐方块重复名称、属性和绝对坐标。写入采用临时文件后原子替换，并对损坏、未知版本和越界坐标提供错误信息。文件字节流由 C++ 解析/生成，不经浏览器构造完整 JSON 工程。

...
用户于 2026-09-07 确定默认二进制工程扩展名为 **`.vmcb`**。具体格式见 [VMCB 1.0](../simulator/docs/vmcbFormat.md)：文件级状态表、16³ 分区、自适应布局编码、独立 Zstandard 压缩及带校验的目录；元数据使用 CBOR。现已接入 C++ 读写和浏览器工程菜单，并兼容旧 JSON 导入；实际验证和规模测量见 [文件实测](../simulator/docs/vmcbFiles.md)。
...
文本审阅表示留给后续独立能力，当前工程只导出 VMCB；默认二进制布局不逐方块重复名称、属性和绝对坐标。当前由 C++ 写完临时成品并关闭后交给浏览器下载，损坏、未知版本和越界坐标均提供错误信息；尚无直接覆盖指定磁盘路径的保存接口。文件字节流由 C++ 解析/生成，不经浏览器构造完整 JSON 工程。

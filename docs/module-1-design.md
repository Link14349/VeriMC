...
**设计工程（暂定 `.verimc`）**：带版本清单的容器格式，包含可读元数据、方块调色板与分区布局、方块实体数据、初始化策略/必要历史、环境配置、输入时间线、探针、分组和视图设置。

**运行快照（暂定 `.verimc-snapshot`）**：记录第 7.4 节的精确执行状态，绑定规则版本和引擎快照版本。跨不兼容版本不强行续跑；提示从设计重新开始或执行显式迁移。

设计可用稳定文本表示导出用于版本控制和审阅；大块重复结构在默认格式中压缩。写入采用临时文件后原子替换，并对损坏、未知版本和越界坐标提供错误信息。

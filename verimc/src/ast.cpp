...
Json toJson(const SourceSpan& s) {
...
Json Diagnostic::json() const {
    Json locations = Json::array();
    for (auto& item : related)
        locations.push_back(toJson(item));
    return {{"code", code},
            {"severity", "error"},
            {"message", what()},
            {"source", toJson(span)},
            {"sourceSha256", sourceSha256.empty() ? Json(nullptr) : Json(sourceSha256)},
            {"instance", instance.empty() ? Json(nullptr) : Json(instance)},
            {"related", locations}};
}
